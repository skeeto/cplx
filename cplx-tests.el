...
(ert-deftest cplx-proj ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-proj c)
                      (cplx-const 0.554700196225229 0.832050294337844)))))

(ert-deftest cplx-sqrt ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-sqrt c)
                      (cplx-const 1.67414922803554 0.895977476129838)))))

(ert-deftest cplx-sin ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-sin c)
                      (cplx-const 9.15449914691143 -4.16890695996656)))))

(ert-deftest cplx-cos ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-cos c)
                      (cplx-const -4.18962569096881 -9.10922789375534)))))

(ert-deftest cplx-tan ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-tan c)
                      (cplx-const -0.00376402564150425 1.00323862735361)))))

(ert-deftest cplx-log ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-log c)
                      (cplx-const 1.28247467873077 0.982793723247329)))))

(ert-deftest cplx-asin ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-asin c)
                      (cplx-const 0.570652784321099 1.98338702991654)))))

(ert-deftest cplx-acos ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-acos c)
                      (cplx-const 1.0001435424738 -1.98338702991654)))))

(ert-deftest cplx-atan ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-atan c)
                      (cplx-const 1.40992104959658 0.229072682968539)))))

(ert-deftest cplx-exp ()
  (let ((c (cplx-const 2.0 3.0)))
    (should (cplx-eql (cplx-exp c)
                      (cplx-const -7.3151100949011 1.0427436562359)))))

(ert-deftest cplx-expt ()
  (let ((a (cplx-const 2.0 3.0))
        (b (cplx-const -2.1 3.2)))
    (should (cplx-eql (cplx-expt a b)
                      (cplx-const -0.0013178979037822 0.00259925004826532)))
    (should (cplx-eql (cplx-expt (cplx-const 0.0 0.0)
                                 (cplx-const 2.0 0.0))
                      (cplx-const 0.0 0.0)))))
